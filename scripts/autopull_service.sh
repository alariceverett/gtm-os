...
LABEL="local.openclaw.autopull"
WORKSPACE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PLIST_TEMPLATE="$WORKSPACE/org/launchd/$LABEL.plist"
PLIST_DIR="$HOME/Library/LaunchAgents"
PLIST_PATH="$PLIST_DIR/$LABEL.plist"
STATUS_JSON="$WORKSPACE/memory/autopull/status.json"
OUT_LOG="$WORKSPACE/memory/autopull/launchd.out.log"
ERR_LOG="$WORKSPACE/memory/autopull/launchd.err.log"
TARGET="gui/$(id -u)"

usage() {
  cat <<EOF
Usage: scripts/autopull_service.sh <command>

Commands:
  install    Install/refresh launchd plist (idempotent)
  start      Install (if needed), load service, and kickstart
  stop       Stop/unload service
  restart    Restart service
  status     Show launchd + runner status
...
ensure_dirs() {
  mkdir -p "$WORKSPACE/memory/autopull" "$PLIST_DIR"
}
