...
- When in doubt, ask.
- **NEVER store credentials in workspace files** (TOOLS.md, MEMORY.md, etc.) — these get injected into session context and can leak to group chats/subagents. Store credentials in `/home/node/.openclaw/.env.*` files instead.
...
- **Run credential audit during heartbeats** — execute `org/security/CREDENTIAL_AUDIT.sh` periodically to catch leaked secrets. Add it to your `HEARTBEAT.md` checklist.
